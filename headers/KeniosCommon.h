...
#define KENIOS_NOTIF_CONFIG_CHANGED @"com.kenios.hax.config.changed"
#define KENIOS_NOTIF_LANGUAGE_CHANGED @"com.kenios.hax.language.changed"

typedef NS_ENUM(NSInteger, KeniosAimTarget) {
    KeniosAimTargetHead = 0, KeniosAimTargetNeck = 1,
    KeniosAimTargetChest = 2, KeniosAimTargetBody = 3
};

typedef NS_ENUM(NSInteger, KeniosAntiBanMode) {
    KeniosAntiBanModeSafe = 0, KeniosAntiBanModeAggressive = 1, KeniosAntiBanModeAuto = 2
};
...
typedef struct {
    uint64_t address; uint64_t meshAddress;
    KeniosVector3 head, neck, chest, pelvis;
    KeniosVector3 leftShoulder, rightShoulder, leftElbow, rightElbow;
    KeniosVector3 leftWrist, rightWrist, leftKnee, rightKnee;
    KeniosVector3 leftFoot, rightFoot;
    KeniosVector2 screenHead, screenChest, screenPelvis;
    float distance, health; int teamID;
    BOOL isBot, isVisible, isAlive, isTeammate, isTarget;
    char name[64];
} KeniosPlayer;

typedef struct {
    KeniosVector3 position; int type;
    float radius, timeToExplode;
    BOOL isActive;
} KeniosBomb;
